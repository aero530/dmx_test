...
pub struct MenuTabData {
    tab0: [(&'static str, ValueType); NUM_ITEMS],
    tab1: [(&'static str, ValueType); NUM_ITEMS],
    tab2: [(&'static str, ValueType); NUM_ITEMS],
}
    ...
    fn from(source: MenuData) -> Self {
        ...
        let (tab1, tab2) = match source.module {
            ...
            ModuleSettings::SmartLed(smart_led_settings) => {
                (
                    [
                        ("Group Mode", ValueType::SmartLedGrouping(smart_led_settings.grouping)),
                        ("Color Mode", ValueType::SmartLedColorMode(smart_led_settings.color_mode)),
                        ("", ValueType::None),
                        ("", ValueType::None),
                        ("", ValueType::None),
                    ],
                    [
                        ("LEDs on Port 1", ValueType::Uint3(smart_led_settings.leds_per_port[0])),
                        ("LEDs on Port 2", ValueType::Uint3(smart_led_settings.leds_per_port[1])),
                        ("LEDs on Port 3", ValueType::Uint3(smart_led_settings.leds_per_port[2])),
                        ("LEDs on Port 4", ValueType::Uint3(smart_led_settings.leds_per_port[3])),
                        ("", ValueType::None),
                    ]
                )
            },
...
impl<'a> Ui<'a> {
    ...
    pub async fn run(&mut self) {
        ...
        // Create menu items
        let menu0_items = [
            MenuItem::new(mtd.tab0[0], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab0[1], false, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab0[2], false, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab0[3], false, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab0[4], false, Point::zero(), text_style.clone()),
        ];

        let menu1_items = [
            MenuItem::new(mtd.tab1[0], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab1[1], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab1[2], false, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab1[3], false, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab1[4], false, Point::zero(), text_style.clone()),
        ];

        let menu2_items = [
            MenuItem::new(mtd.tab2[0], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab2[1], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab2[2], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab2[3], true, Point::zero(), text_style.clone()),
            MenuItem::new(mtd.tab2[4], false, Point::zero(), text_style.clone()),
        ];
                    ...
                    match new_message {
                        ...
                        UiEvent::Select => {
                            if self.menus[self.current_tab].editing() {
                                ...
                                match self.tx.try_send(RouterEvent::StoreSettingsInEeprom(self.menu_data)) {
                                    Ok(_) => {}
