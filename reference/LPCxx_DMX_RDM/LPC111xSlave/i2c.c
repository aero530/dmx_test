...
#else
static volatile bool I2C_SemWait;
#endif
...
i2c_error_t I2C_Init(uint32_t mode, uint8_t address, void *pFuncRecv, void *pFuncXmit)
{
    ...
    NVIC_SetPriority (I2C_IRQn, I2C_IRQ_PRIORITY);
#ifdef USE_RTK
    I2C_SemWait = rtk_sem_create( 0, "I2Cwait" );
    I2C_SemBusy = rtk_sem_create( 1, "I2Cbusy" );
#else
    // Warning: No re-entrance protection when no RTK is used!!!!
    I2C_SemWait = false;
#endif
    NVIC_DisableIRQ(I2C_IRQn);
    /*--- Clear all I2CON flags ---*/
    LPC_I2C->CONCLR = I2CONCLR_I2ENC | I2CONCLR_STAC | I2CONCLR_SIC | I2CONCLR_AAC;
#ifdef SUPPORT_I2C_MASTER_MODE
    I2C_Error = I2C_OK;
#endif
    ...
    if (mode == I2CSLAVE) {
        ...
        LPC_I2C->ADR0  = (address & 0xFE);  /* Gencall bit is disabled! */
...
void I2C_IRQHandler(void)
{
    ...
    case I2STAT_M_REPEATED_START:
        ...
        LPC_I2C->DAT    = I2C_DeviceAddress;
        LPC_I2C->CONSET = I2CONSET_AA;
        LPC_I2C->CONCLR = I2CONCLR_STAC | I2CONCLR_SIC;
        break;

    case I2STAT_M_ARB_LOST:
        I2C_Error = I2C_ERROR_ARB_LOST;
        LPC_I2C->CONCLR = I2CONCLR_SIC; /* no repeated start implemented! */
        NVIC_DisableIRQ(I2C_IRQn);
#ifdef USE_RTK
    ...
    case I2STAT_M_TX_DAT_ACK:
        if (I2C_UseRegNum == TRANSMIT_REGNUM) {
            ...
            LPC_I2C->DAT    = I2C_RegNum;
            LPC_I2C->CONSET = I2CONSET_AA;
            LPC_I2C->CONCLR = I2CONCLR_SIC;
        } else if (I2C_TRxCount < I2C_BufCount) {
            LPC_I2C->DAT    = *(I2C_BufPtr + I2C_TRxCount);
            I2C_TRxCount++;
            LPC_I2C->CONSET = I2CONSET_AA;
            LPC_I2C->CONCLR = I2CONCLR_SIC;
        } else {
            LPC_I2C->CONSET = I2CONSET_STO | I2CONSET_AA;
            LPC_I2C->CONCLR = I2CONCLR_SIC;
            NVIC_DisableIRQ(I2C_IRQn);
#ifdef USE_RTK
            rtk_sem_release_isr( I2C_SemWait );
    ...
    case I2STAT_M_RX_DAT_ACK:
        *(I2C_BufPtr + I2C_TRxCount) = LPC_I2C->DAT;
        I2C_TRxCount++;
        if ((I2C_TRxCount + 1) <= I2C_BufCount) {
            LPC_I2C->CONSET = I2CONSET_AA;
            LPC_I2C->CONCLR = I2CONCLR_SIC;
    ...
    case I2STAT_S_RX_GENCALL_DAT_ACK:
        if (I2C_UseRegNum == RECEIVE_REGNUM) {
            ...
            I2C_RegNum    = LPC_I2C->DAT;
        } else {
            uint8_t RecvData = LPC_I2C->DAT;
    ...
    case I2STAT_S_TX_DAT_ACK_FINISH:
        ...
        LPC_I2C->DAT    = XmitData;  /* write data back to i2c master */
        LPC_I2C->CONSET = I2CONSET_AA;
	    LPC_I2C->CONCLR = I2CONCLR_SIC;
        I2C_TRxCount++;
        break;
